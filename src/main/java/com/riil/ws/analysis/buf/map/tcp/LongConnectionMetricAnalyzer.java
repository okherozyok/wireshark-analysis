...
public class LongConnectionMetricAnalyzer {
    ...
    void start(TcpStream tcpStream) {
        ...
        tcpStream.setLongConnMetricStartTime(firstFrame.getTimestamp());
    }

    void every(TcpStream tcpStream, FrameBean frame) {
        while (frame.getTimestamp() >= (tcpStream.getLongConnMetricStartTime() + MINUTE_BY_MS)) {
            ...
            tcpStream.setLongConnMetricStartTime(tcpStream.getLongConnMetricStartTime() + MINUTE_BY_MS);
        }
